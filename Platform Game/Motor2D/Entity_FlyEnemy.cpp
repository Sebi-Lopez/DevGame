...
Entity_FlyEnemy::Entity_FlyEnemy(int x, int y, pugi::xml_node& node) : j1Entity(x, y)
{
	...
	sprites = App->tex->Load("textures/Fly_enemy.png");
	...
	SetAnimation(animations.child("flyanimationinpath").child("anim"), flyinpath);
	flyinpath.speed = animations.child("flyanimationinpath").attribute("speed").as_float();
	flyinpath.loop = animations.child("flyanimationinpath").attribute("loop").as_bool();
...
void Entity_FlyEnemy::SetPlayerState()
{
...
void Entity_FlyEnemy::SetPlayerActions()
{
	switch (State)
	{
	case STATE::IDLE:
	...
	case STATE::FLYING_FORWARD:
		velocity.x = run_speed;
		animation = &run;
		break;

	case STATE::FLYING_BACKWARD:
		velocity.x = -run_speed;
		animation = &run;
		break;
...
void Entity_FlyEnemy::SetAnimation(pugi::xml_node& node, Animation& anim) {
	SDL_Rect components;
...
{
	pugi::xml_node load = node.child("flyenemyattributes");
	loadpos = true;
...
bool Entity_FlyEnemy::Save(pugi::xml_node& node) const {
	pugi::xml_node save = node.append_child("flyenemyattributes");
